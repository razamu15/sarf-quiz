// Quiz generator. buildQuiz(settings) → array of question objects:
...
const BUILDERS = {
  ...
  tense(settings) {
    ...
    return {
      ...
      explanation: `${v.word} — ${correct.ar} from ${citation(v.rootEntry, v.formId)} (${glossOf(v.rootEntry, v.formId)}).`,
    };
  },

  voice(settings) {
    // only pick verbs where the majhūl actually exists, so both answers are live
    for (let i = 0; i < 60; i++) {
      const v = randomVerb(settings, { tenses: ['madi', 'mudari'] });
      if (!v) return null;
      if (!conjugate(v.rootEntry, v.formId, v.tense, 'majhul', v.slot)) continue;
      const correct = VOICE_LABELS[v.voice];
      const options = shuffle(Object.values(VOICE_LABELS));
      const waznWord = waznOf(v.formId, v.tense, v.voice, v.slot, v.rootEntry.forms[v.formId].bab ?? 1);
      return {
        category: 'voice',
        word: v.word,
  ...
  doer(settings) {
    ...
    const others = sample(slots, 8, (s) => s !== v.slot)
