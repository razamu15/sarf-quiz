...
**The stats strip tells the truth for v1.** `detailedStats` is off, so there is no chevron into a screen that is not there: three numbers (streak, questions this week, accuracy) and seven bars for the week, all computed from stored answers by `basicSummary()`. No flame, no ring. When the Pro screens land, the strip becomes the way in — and by then it covers the user's whole history, which is the point of storing everything from the first build.
